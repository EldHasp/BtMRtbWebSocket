...
namespace BitMexLibrary.Enums
{
    public static class EnumExtensions
    {
        public static SideEnum ToSideEnum(this string value)
        {
            if (Enum.TryParse(value, out SideEnum _val))
                return _val;
            return SideEnum.UnKnown;
        }

        public static SideEnum ToSideEnum(this object value)
        {
            if (value is string valStr)
            {
                if (Enum.TryParse(valStr, out SideEnum _val))
                    return _val;
                return SideEnum.UnKnown;
            }
            if (value is int valInt && Enum.IsDefined(typeof(SideEnum), valInt))
                return (SideEnum)valInt;
            return SideEnum.UnKnown;
        }

        public static SideEnum GetHand(this int value)
        {
            if (value == 0)
                return SideEnum.UnKnown;
            if (value > 0)
